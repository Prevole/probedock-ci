...
sudo -E docker run --rm  --net ${PROBEDOCK_ENV}-probenet --link $FIRST_CONTAINER:db aanand/wait
...
sudo -E docker run --rm --net ${PROBEDOCK_ENV}-probenet --link $FIRST_CONTAINER:cache aanand/wait