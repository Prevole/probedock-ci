...
sudo -E docker-compose -p $PROBEDOCK_ENV run waitdb
#sudo -E docker run --rm --net=${PROBEDOCK_ENV}_probenet --link $FIRST_CONTAINER:db aanand/wait
...
sudo -E docker run --rm --net=${PROBEDOCK_ENV}_probenet --link $FIRST_CONTAINER:cache aanand/wait